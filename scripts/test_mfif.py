"""
test "matrix factorization item finders"

Description
===========

Input events are explicitly rated, but only preferred items are treated as if
they are imlicitly rated in training stage.
...
-n, --no-timestamp or --timestamp
    ...
    default=timestamp
-r, --rthresh <RTHRESH>
    A rating threshold, if larger than which if ratings of items are larger or
     equal, then the items are treated as implicitly preferred.   
...
def training(opt, ev, tsc, event_feature=None, fold=0):
    ...
    data.filter_event(data.score >= opt.rsresh)
...
def testing(rcmdr, fp, opt, ev, tsc, ts=None, fold=0):
    ...
    if ts is None:
        ...
        for i in xrange(ev.shape[0]):
            print(ev[i, 0], ev[i, 1],
                  0 if tsc[i] < opt.rthresh else 1, esc[i],
                  file=fp, sep='\t')
    else:
        if opt.header:
            print('User ID', 'Item ID', 'Original Score', 'Predicted Score',
        ...
        for i in xrange(ev.shape[0]):
            print(ev[i, 0], ev[i, 1],
                  0 if tsc[i] < opt.rthresh else 1, esc[i], ts[i],
                  file=fp, sep='\t')
...
if __name__ == '__main__':
    ...
    ap.add_argument('-r', '--rthresh', type=float, default=4)
